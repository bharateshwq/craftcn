...
export const useIframeCommunication = ({
  ...
  // Handle iframe load
  useEffect(() => {
    console.log('store-front-editor iframe load useEffect', ref.current)
    const iframe = ref.current
    if (!iframe) {
      setStatus('unknown')
      return
    ...
    const handleLoad = () => {
      console.log('store-front-editor iframe load handleLoad')
      if (allowCrossOrigin) {
    ...
    //Below are two ways to invoke the handleLoad to manage race conditions

    //this handle load runs when iframe is already loaded
    if (iframe.src && iframe.contentDocument?.readyState === 'complete') {
      handleLoad()
    }

    //this handle load runs when iframe is loaded
